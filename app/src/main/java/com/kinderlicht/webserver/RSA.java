...
public class RSA {
	...
	private static final BigInteger[] KEYS = calculateKeys();
	
	public static BigInteger[] getPublicKey() {
		BigInteger[] pub = {KEYS[0], KEYS[2]};
		return pub;
	}
	
	public static BigInteger[] encrypt(String message, BigInteger[] keys) {
	...
	public static BigInteger[] extendedEuclid(BigInteger a, BigInteger b) {
		...
		if (a.equals(BigInteger.ZERO)) {
			...
			gdc[0] = b;
			gdc[1] = BigInteger.ZERO;
			gdc[2] = BigInteger.ONE;
			return gdc;
		}
		BigInteger[] x = extendedEuclid(b.mod(a), a);
		BigInteger[] gdc = new BigInteger[3];
		gdc[0] = a;
		gdc[1] = x[2].subtract(b.divide(a).multiply(x[1]));
		gdc[2] = x[1];
		return gdc;
	}
